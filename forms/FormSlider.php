...
class FormSlider extends \Widget
{
    ...
    public function __construct($arrAttributes = null)
    {
        ...
        foreach ($arrSliderConfig as $key => $value) {
            if (empty($value) || StringUtil::endsWith($key, '_callback')) {
                continue;
            }
    ...
    protected function getSliderConfig($arrConfig = [])
    {
        ...
        if ($arrConfig['range'] == true) {
            ...
            $min                = $arrRange[0] >= $min ? $arrRange[0] : $min;
            $max                = $arrRange[1] <= $max ? $arrRange[1] : $max;
        } else {
            $max                = $this->varValue <= $max ? $this->varValue : $max;
            $arrConfig['value'] = $max;
        }


        if (isset($arrConfig['rangeLabels'])) {
